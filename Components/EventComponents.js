...
export async function makeNewEvent(title, information, startTime, endTime, programID) {
    ...
    const res = await db.collection('Events').doc(eventID.NextPostID.toString()).set(data);
...
async function getNextPost() {
    ...
    if (!snapshot.exists) {
        ...
        // Multiple events, posts, etc. may end up having the same ID. Firebase has an API increment
